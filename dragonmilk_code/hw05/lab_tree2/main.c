...
void createHTree()
{
	struct tnode *p = NULL,*q = NULL , *r = NULL,*que[2048];
	int i = 0 , j = 0 ,top = 0 , L = 0 , R = 0;
	for(i = 0 ; i < 128 ; ++i)
	{
		if(Ccount[i] != 0) 
		{
			p = (struct tnode*) malloc(sizeof(struct tnode));
			p->c = (char)i , p->left = p->right = NULL,p->weight = Ccount[i];
			que[R++] = p;
		}
	}
	while(R-L > 1)
	{
		...
		for(i = L ; i < R ; ++i)
		{
			for(j = R-1 ; j > i ; --j)
			{
				if(que[j-1]->weight > que[j]->weight) p = que[j-1],que[j-1] = que[j],que[j] = p;
				else if(que[j-1]->weight == que[j]->weight && que[j-1]->c > que[j]->c) p = que[j-1],que[j-1] = que[j],que[j] = p;
			}
		}		
		p = que[L++];
		q = que[L++];
		r = (struct tnode*) malloc(sizeof(struct tnode));
		r->left = p,r->right = q,r->weight = p->weight+q->weight,r->c = 127,que[R++] = r;
		for(j = L ; j < R ; ++j) printf("%c:%d ",que[j]->c,que[j]->weight);
		printf("\n");
	}
	Root = que[L];
}
