...
tnode* buildTnode(ele element) {
	tnode* cur = (tnode*)malloc(sizeof(tnode));
	cur->item = element;
	cur->l = cur->r = NULL;
	return cur;
}
tnode* buildUnitTree(tnode* fir, tnode* sec, tnode* op) {
	op->l = fir;
	op->r = sec;
	return op;
}
...
void push_t(Stk_t* stkname, tnode* cur) {
	stkname->stk_t[stkname->top++]= cur;
}
tnode* getTop_t(Stk_t* stkname) {
	//获取栈顶的元素
	return stkname->stk_t[stkname->top - 1];
}

ele buildEle(int type, int data) {
	ele returnEle;
	returnEle.type = type;
	if (type == IS_SIGN) {
...
int main()
{
	...
	for (int i = 0; i < exRes.top; ++i) {
		ele cur = exRes.stk[i];
		if (cur.type == IS_SIGN) {
			tnode* op=buildTnode(cur);
			tnode* sec = getTop_t(&treeRes);
			pop_t(&treeRes);
			tnode* fir =getTop_t(&treeRes);
			pop_t(&treeRes);
			push_t(&treeRes, buildUnitTree(fir, sec, op));
		}
		else {
			//操作数
			push_t(&treeRes, buildTnode(cur));
		}
	}
	tnode* head = treeRes.stk_t[0];
	PrintEle(head->item);
	PrintEle(head->l->item);
	PrintEle(head->r->item);
	printf("\n");
	printf("%d", ans.stk[0].data);
	release(head);
